...
> Cập nhật 17/06/2026 — production chạy trên VNG AgentBase: image `v36`, runtime version 49, UI cache `fix-20260617s`, storage `cloud`, mode `remote_agents`, public-review lock ON.
...
Toàn bộ app nằm trong **một Docker image Python duy nhất** — backend `ThreadingHTTPServer` không framework, UI HTML/CSS/vanilla JS không build step.
...
User / Reviewer  ──▶  AgentBase Runtime (orchestrator, image v35)
                         ...
                         ├── AgentBase Memory stores (riêng từng analysis agent)
                         └── AgentBase MCP Gateway
```

Production chạy **remote multi-agent**: orchestrator nhận request, gọi 4 child runtime độc lập qua `POST /invocations`, mỗi child có runtime riêng, model riêng, **knowledge store riêng** và tự semantic-recall trước khi trả về. Memory insight + executive summary tổng hợp ở orchestrator. Child lỗi thì orchestrator fallback theo từng agent, không làm hỏng cả flow. Trace production thể hiện `orchestration.mode=remote_agents` + `ragSources.storeId` trên từng child để chứng minh điều này.

MCP còn có **channel skill self-host** cho OpenClaw/Zalo/Telegram/Discord (`/api/channel-skill`, `/openclaw/skill`, `/discord/skill`, kèm system-prompt và `mcp-remote.json`), nên bot có thể điều khiển LaunchOps mà không cần AgentBase Gateway.
...
| **Kiến trúc MCP** | ✅ MCP server + channel skill cho nhiều kênh chat/agent | `/mcp` + channel skill OpenClaw/Zalo/Telegram/Discord |
...
OpenClaw kết nối qua `npx mcp-remote <endpoint>/mcp`. Self-host: trỏ bot về `/mcp` hoặc `POST /tools/call`.
...
server/test_app.py      # unit test stdlib (152 test)
server/seed_knowledge.py · server/migrate_to_cloud_db.py
data/ · prompts/ · Dockerfile · README_EN.md
```

## Test

```bash
python -m unittest server.test_app    # 152 test, stdlib, không cần .env
